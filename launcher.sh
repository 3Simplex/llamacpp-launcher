...
launch_server() {
    ...
    local cmd_args=("-m" "$SELECTED_MODEL_PATH" "-c" "$USABLE_CONTEXT" "--host" "0.0.0.0" "--port" "$port" "--alias" "$alias_name" "--props" "--metrics" "-ctk" "$cache_type" "-np" "$parallel_slots" -t "$threads" "-fa" "on" "--no-mmap" "--ubatch" "2048")
...
launch_from_preset() {
    ...
    local cmd_args=("-m" "$model_path" "-c" "$context" "--host" "0.0.0.0" "--port" "$port" "--alias" "$alias_name" "--props" "--metrics" "-ctk" "$cache_type" "-np" "$parallel" -t "$threads" "-fa" "on" "--no-mmap" "--ubatch" "2048")
