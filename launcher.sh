...
launch_server() {
    ...
    local cmd_args=("-m" "$SELECTED_MODEL_PATH" "-c" "$USABLE_CONTEXT" "--host" "0.0.0.0" "--port" "$port" "--alias" "$alias_name" "--props" "--metrics" "-np" "$parallel_slots" -t "$threads")
...
launch_from_preset() {
    ...
    local cmd_args=("-m" "$model_path" "-c" "$context" "--host" "0.0.0.0" "--port" "$port" "--alias" "$alias_name" "--props" "--metrics" "-np" "$parallel" -t "$threads")
